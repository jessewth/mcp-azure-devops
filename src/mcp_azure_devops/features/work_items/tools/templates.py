...
def _get_work_item_templates_impl(
    ...
    try:
        ...
        if not templates:
            scope = (
                f"work item type '{work_item_type}' in "
                if work_item_type
                else ""
            )
        ...
        # Create header
        project_display = team_context.get("project") or team_context.get(
            "project_id"
        )
        header = (
            f"# Work Item Templates for Team: {team_display} "
...
def _get_work_item_template_impl(
    team_context: dict, template_id: str, wit_client: WorkItemTrackingClient
) -> str:
...
def register_tools(mcp) -> None:
    ...
    def get_work_item_templates(
        team_context: dict, work_item_type: Optional[str]
    ) -> str:
        """
