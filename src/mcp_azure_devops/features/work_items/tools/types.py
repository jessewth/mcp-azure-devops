...
def _format_work_item_type(wit):
    ...
    if states:
        ...
        for state in states:
            state_info = (
                f"- {state.name} (Category: {state.category}, "
                f"Color: {state.color})"
            )
...
def _get_work_item_types_impl(
    project: str, wit_client: WorkItemTrackingClient
) -> str:
...
def _get_work_item_type_impl(
    project: str, type_name: str, wit_client: WorkItemTrackingClient
) -> str:
...
def _get_work_item_type_fields_impl(
    project: str, type_name: str, wit_client: WorkItemTrackingClient
) -> str:
    ...
    try:
        ...
        if not wit:
            return (
                f"Work item type '{type_name}' not found in project {project}."
            )
        ...
        if not fields:
            return (
                f"No fields found for work item type '{type_name}' "
        ...
        return (
            f"Error retrieving fields for work item type '{type_name}' "
...
def _get_work_item_type_field_impl(
    ...
    try:
        ...
        if "." not in field_name:
            ...
            if not field_ref:
                return (
                    f"Field '{field_name}' not found for work item type "
        ...
        return (
            f"Error retrieving field '{field_name}' for work item type "
...
def register_tools(mcp) -> None:
    ...
    def get_work_item_type_field(
        project: str, type_name: str, field_name: str
    ) -> str:
        """
