...
def test_create_work_item_impl_success():
    ...
    # description 應自動轉為 HTML 格式
    assert any(op["path"] == "/fields/System.Description" and op["value"] == "<div>純文字描述<br>第二行</div>" for op in document)
...
def test_create_work_item_impl_with_area_and_iteration():
    ...
    assert any(op["path"] == "/fields/System.Title" and op["value"] == "測試標題" for op in document)
    # description 應自動轉為 HTML 格式
    assert any(op["path"] == "/fields/System.Description" and op["value"] == "<div>描述內容<br>第二行</div>" for op in document)
