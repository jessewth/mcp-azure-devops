...
def test_get_project_process_id_impl_error(mock_get_core_client):
    ...
    # Assert
    assert (
        "Error retrieving process ID for project 'Test Project': "
        "Test error" in result
    )


@patch(
    "mcp_azure_devops.features.work_items.tools.process.get_work_item_tracking_process_client"
)
def test_get_process_details_impl(mock_get_process_client):
    ...
    # Assert
    mock_process_client.get_process_by_its_id.assert_called_once_with(
        "process-id-123"
    )
    mock_process_client.get_process_work_item_types.assert_called_once_with(
        "process-id-123"
    )
...
def test_get_process_details_impl_error(mock_get_process_client):
    ...
    # Assert
    assert (
        "Error retrieving process details for process ID 'process-id-123':"
        " Test error" in result
    )
