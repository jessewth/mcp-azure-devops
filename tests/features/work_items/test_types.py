...
def test_get_work_item_type_impl_not_found():
    ...
    assert (
        "Work item type 'NonExistentType' not found in project "
...
@patch(
    "mcp_azure_devops.features.work_items.tools.types.get_work_item_tracking_process_client"
)
def test_get_work_item_type_fields_impl(
    ...
    mock_project.capabilities = {
        "processTemplate": {"templateTypeId": "process-id-123"}
    }
...
def test_get_work_item_type_fields_impl_no_fields(
    ...
    # Assert
    assert (
        "No fields found for work item type 'Bug' in project TestProject"
        in result
    )
